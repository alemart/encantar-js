...
export class Watermark
{
    ...
    private _createDialog(): Nullable<HTMLDialogElement>
    {
        ...
        dialog.style.backgroundColor = 'whitesmoke';
        dialog.style.color = '#333';
        dialog.style.padding = '0';
        dialog.style.boxShadow = '4px 4px 20px rgba(0,0,0,0.75)';
        dialog.style.textAlign = 'center';
        dialog.style.fontSize = '16px';
        dialog.style.fontFamily = '"Lato", sans-serif';
        ...
        copyrightText.style.margin = '12px 0';
        wrapper.appendChild(copyrightText);

        aboutText.innerText = 'This AR experience was created using encantar.js, a high performance Augmented Reality framework for the web released as open source software under the GNU Lesser General Public License version 3.';
        ...
        const highlight = (el: HTMLElement, bg: string, fg: string) => (() => {
            el.style.backgroundColor = bg;
            el.style.color = fg;
            el.style.border = '2px solid ' + (bg !== 'transparent' ? bg : fg);
        });

        infoButton.style.cursor = 'pointer';
        infoButton.style.fontSize = '18px';
        infoButton.style.fontWeight = 'bold';
        infoButton.style.fontFamily = '"Lato", sans-serif';
        infoButton.style.color = 'blueviolet';
        infoButton.style.backgroundColor = 'transparent';
        infoButton.style.border = '2px solid blueviolet';
        ...
        infoButton.innerText = 'More about encantar.js';
        infoButton.addEventListener('click', () => location.href = WEBSITE);
        infoButton.addEventListener('pointerdown', highlight(infoButton, 'gold', 'black'));
        infoButton.addEventListener('pointerup', highlight(infoButton, 'transparent', 'blueviolet'));
        infoButton.addEventListener('pointerleave', highlight(infoButton, 'transparent', 'blueviolet'));
        buttonWrapper.appendChild(infoButton);

        okButton.style.cursor = 'pointer';
        okButton.style.fontSize = '18px';
        okButton.style.fontWeight = 'bold';
        okButton.style.fontFamily = '"Lato", sans-serif';
        okButton.style.color = 'white';
        okButton.style.backgroundColor = 'blueviolet';
        okButton.style.border = '2px solid blueviolet';
        okButton.style.padding = '12px 24px';
        okButton.style.marginLeft = '12px';
        okButton.style.outline = 'none';
        (okButton.style as any)['-webkit-tap-highlight-color'] = 'transparent';
        okButton.innerText = 'OK';
        okButton.setAttribute('autofocus', '');
        okButton.addEventListener('click', () => dialog.close());
        okButton.addEventListener('pointerdown', highlight(okButton, 'gold', 'black'));
        okButton.addEventListener('pointerup', highlight(okButton, 'blueviolet', 'white'));
        okButton.addEventListener('pointerleave', highlight(okButton, 'blueviolet', 'white'));
